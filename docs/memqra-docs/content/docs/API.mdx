---
title: "API reference"
description: "The full MemoryEngine API surface: createMemoryEngine and every method it returns."
---
...
[docs/ADAPTERS](/docs/ADAPTERS)). See `README.md` "Roadmap" for what's designed but
not yet built.
...
query text — see [docs/SEARCH](/docs/SEARCH).
...
- `evolve(id, patch, options?)` — alias for `update()`. The primitive
  `ingest()` is built on. See [docs/MEMORY-EVOLUTION](/docs/MEMORY-EVOLUTION).
...
- `search(query: SearchQuery): Promise<SearchResult[]>` — runs the hybrid
  retrieval pipeline ([docs/SEARCH](/docs/SEARCH)): structured filtering always
  ...
  [docs/ARCHITECTURE](/docs/ARCHITECTURE), "graceful degradation").
...
See [docs/GRAPH](/docs/GRAPH) for full details. All require a `GraphStore`; throw
...
See [docs/MEMORY-EVOLUTION](/docs/MEMORY-EVOLUTION) for full details.
...
See [docs/MAINTENANCE](/docs/MAINTENANCE) for full details.
...
See [docs/OBSERVABILITY](/docs/OBSERVABILITY) and [docs/SECURITY](/docs/SECURITY). Configured via
