...
description: "Identity vs. state vs. version, and the ingest pipeline: identity resolution, duplicates, and conflicts."
---
...
- **`engine.evolve(id, patch, options?)`** — the primitive. You already
  ...
  an immutable `MemoryVersion` snapshot. This is what [docs/MEMORY-LIFECYCLE](/docs/MEMORY-LIFECYCLE)
...
Per "no fake implementations" (spec section 77 / [docs/CONTRIBUTING](/docs/CONTRIBUTING)):
